...
filters = list(set(filters).intersection(['B', 'V', 'g', 'i']))
...
def log_likelihood(theta, data):
    ...
    if (Mzams_range[0] <= theta[0] <= Mzams_range[-1]) & \
        ...
        log_likeli += log_likelihood_lum / len(x_moved_lum)  # normalized to number of lum observations
        ...
        log_likeli += log_likelihood_veloc / len(x_moved_veloc)  # normalized to number of veloc observations
                                                                # for equal weight of each observation type
        ...
        for filt in filters:
            ...
            log_likeli += log_likelihood_mag / len(x_moved_mag)  # normalized to number of mag observations
                                                                # for equal weight of each observation type
...
def plot_lightcurve_with_fit(data, sampler, step):
    ...
    # add log likelihood for luminosity
    log_likelihood_lum = -np.sum((data['lum']['Lum'] - y_fit_lum) ** 2. /
                                 (2. * data['lum']['dLum0'] ** 2.) + np.log(data['lum']['Lum'])) / len(x_moved_lum)
    log_likeli += log_likelihood_lum
    print('log likelihood mag', log_likelihood_lum)
    # add log likelihood for velocity
    log_likelihood_veloc = -np.sum((data['veloc']['veloc'] - y_fit_veloc) ** 2. /
                                   (2. * data['veloc']['dveloc'] ** 2.) + np.log(data['veloc']['veloc'])) / len(x_moved_veloc)
    ...
    for filt in filters:
        ...
        log_likelihood_mag_filt[filt] = -np.sum((data_filt_y - y_fit_mag_filt) ** 2. / (2. * data_filt_dy ** 2.)
                                          + np.log(data_filt_y)) / len(x_moved_mag)
