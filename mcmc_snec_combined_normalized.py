...
def log_likelihood(theta, data):
    log_likeli = 0
    # normalization to number of observations for equal weight of each observation type
    ...
    log_likeli += mag.log_liklihood(theta, data['mag']) / num_obs_mag
    log_likeli += lum.log_liklihood(theta, data['lum']) / num_obs_lum
    log_likeli += veloc.log_liklihood(theta, data['veloc']) / num_obs_veloc
    return log_likeli
...
def plot_lightcurve_with_fit(SN_data, sampler, step):
    ...
    log_likeli += mag.plot_lightcurve_with_fit(SN_data['mag'], sampler, step) / num_obs_mag
    log_likeli += lum.plot_lightcurve_with_fit(SN_data['lum'], sampler, step) / num_obs_lum
    log_likeli += veloc.plot_lightcurve_with_fit(SN_data['veloc'], sampler, step) / num_obs_veloc
...
log_likelihood_all = plot_lightcurve_with_fit(SN_data_all_w_early, sampler, n_steps-1)
# log_likelihood_all = plot_lightcurve_with_fit(SN_data_all_w_early, sampler, 50)
# log_likelihood_all = plot_lightcurve_with_fit(SN_data_all_w_early, sampler, 20)
log_likelihood_all = plot_lightcurve_with_fit(SN_data_all_w_early, sampler, 1)
