...
def plot_lum_with_fit(data_lum, sampler_chain, ranges_dict, step, output_dir, n_walkers, SN_name):
    ...
    num_obs_lum = len(data_lum['t_from_discovery'])
    print('num_obs_lum', num_obs_lum)
    for i in range(n_walkers):
        ...
        if theta_in_range(requested, ranges_dict) or R == 0:
            ...
            if not isinstance(y_fit, str):
                ...
                log_likeli.append(calc_lum_likelihood(requested, data_lum, ranges_list) / num_obs_lum)
    ...
    ax.set_title('step ' + str(step)
                 + '\nnormalized log likelihood = ' + str(int(log_likeli)), fontsize=14)
    plt.tight_layout()
...
def plot_mag_with_fit(data_mag, sampler_chain, ranges_dict, step, output_dir, n_walkers, SN_name):
    ...
    for i in range(n_walkers):
        ...
        if theta_in_range(requested, ranges_dict) or R == 0:
            ...
            if not isinstance(y_fit, str):
                num_obs_mag = len(data_mag['t_from_discovery'])
                print('num_obs_mag', num_obs_mag)
                # multiply whole graph by scaling factor
                y_fit = y_fit * S
                for filt in filters:
                    ax.plot(x_plotting, y_fit[filt], color=colors[filt], alpha=0.3)
                log_likeli.append(calc_mag_likelihood(requested, data_mag, ranges_list) / num_obs_mag)
    log_likeli = np.average(log_likeli)
    for filt in filters:
    ...
    for i in range(n_walkers):
        ...
        if theta_in_range(requested, ranges_dict) or R == 0:
            if Tthresh_normalized:
                ...
                num_obs_veloc = len(thresh_veloc['t_from_discovery'])
                print('num_obs_veloc', num_obs_veloc)
                ...
                num_obs_veloc = len(data_veloc['t_from_discovery'])
                data_x_moved = x_plotting - T
                y_fit = interp_veloc.snec_interpolator(requested[0:6], ranges_list, data_x_moved)
                if not isinstance(y_fit, str):
                    # multiply whole graph by scaling factor
                    y_fit = y_fit * Sv
                    ax.plot(x_plotting, y_fit, alpha=0.4)
                    log_likeli.append(calc_veloc_likelihood(requested, data_veloc, ranges_list) / num_obs_veloc)
    log_likeli = np.average(log_likeli)
