...
def plot_lum_with_fit(data, sampler_df, ranges_dict, output_dir, n_walkers, SN_name, martinez_values):
    ...
    ax.text(1.02, 0.73, results_text, transform=ax.transAxes, fontsize=14,
    ...
    ax.set_yscale('log')
    plt.tight_layout()
    f_fit.savefig(os.path.join(output_dir, str(result_t) + '_wveloc_S1.4_' + SN_name + '_lum_log.png'))
...
def plot_veloc_with_fit(data, sampler_df, ranges_dict, output_dir, n_walkers, SN_name, martinez_values):
    ...
    ax.set_title(SN_name, fontsize=18)
    plt.tight_layout()
    f_fit.savefig(os.path.join(output_dir, str(result_t) + '_wveloc_S1.4_' + SN_name + '_veloc.png'))
