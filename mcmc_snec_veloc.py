...
run_param_df = pd.DataFrame.from_dict({'Mzams_range': str(Mzams_range), 'Ni_range': str(Ni_range),
                             ...
                             'Mix_range': str(Mix_range), 'T_exp_min15_range': str(T_range),
...
SN = pd.read_csv(os.path.join('results', SN_name+'_expansion_velocities.csv'),
                      usecols=['t_from_discovery', 'line', 'absorption_mean_velocity','absorption_std_velocity'])
SN.rename({'absorption_mean_velocity':'veloc', 'absorption_std_velocity':'dveloc'}, axis='columns', inplace=True)
SN = SN.loc[SN['line'] == 'FeII 5169']
SN.sort_values(by=['t_from_discovery'], inplace=True)
# remove first point which seems like an artifact
SN = SN.loc[SN['t_from_discovery'] > 20]
...
def log_prior(theta):
    ...
    # T of explosion (T)
    if T_range[0] <= theta[6] <= T_range[-1]:
        prob_T = 1. / theta[6]
    else:
        prob_T = 0.

    # sum probabilities
    prob_total = np.log(prob_M * prob_Ni * prob_E * prob_R * prob_K * prob_Mix * prob_T)
    return prob_total
...
def log_likelihood(theta, data):
    ...
    if (Mzams_range[0] <= theta[0] <= Mzams_range[-1]) &\
            ...
            (T_range[0] <= theta[6] <= T_range[-1]):
        data_x_moved = data['t_from_discovery'] - 15 + theta[6]
        data_y = data['veloc']
        data_dy = data['dveloc']
...
def emcee_fit_params(data):
    ...
    initial_guesses = np.array([Mzams_random, Ni_random, E_random, R_random, K_random, Mix_random, T_random])
...
def get_param_results_dict(sampler, step):
    ...
    # df.to_csv(os.path.join(res_dir, 'final_results.csv'))
    return dict
