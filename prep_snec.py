...
def create_directory(i_ni_mass, i_ni_mix, imass, ienergy, idensity, iradius, overwrite=False):
    ...
    path = os.path.join('snec_models',
                        ...
                        'M{:2.1f}'.format(imass),
                        # TODO - figure out formating so if string is single digit, prepended with a 0
...
def write_directory_list(path_list):
    ...
    for ipath in path_list:
        ofile.write('{}\n'.format(ipath))
    ofile.close()
    os.chmod('input_dir_list.txt'.format(array_num),
             stat.S_IRUSR | stat.S_IRGRP | stat.S_IROTH | stat.S_IWUSR | stat.S_IXUSR)
...
def write_sbatch_job(timeout, array_num):
    ...
    #SBATCH --mail-user=shaharbra@gmail.com 
    ...
    mkdir -p /scratch/shahar/job_$SLURM_ARRAY_TASK_ID
    cp -r $SEED/* /scratch/shahar/job_$SLURM_ARRAY_TASK_ID
    cd /scratch/shahar/job_$SLURM_ARRAY_TASK_ID
    srun ./snec &>snec.out   
    #make sure you captured all your data
    cp /scratch/shahar/job_$SLURM_ARRAY_TASK_ID/snec.out $SEED
    cp -r /scratch/shahar/job_$SLURM_ARRAY_TASK_ID/Data $SEED/Data
    rm -rf /scratch/shahar/job_$SLURM_ARRAY_TASK_ID
                ...
                for indx_energy, ienergy in enumerate(parameters['explosion_energy']):
                    for indx_density, idensity in enumerate(parameters['density_1D']):
                        for indx_radius, iradius in enumerate(parameters['wind_extent']):
                            basepath = create_directory(i_ni_mass, i_ni_mix, imass, ienergy, idensity, iradius,
                                                        overwrite=False)
