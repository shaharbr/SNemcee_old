...
T_thresh = 10 ** 3.75
result_name = '2021-02-15_19-40-02_lum_veloc_w_Sv_SN2004a'
...
def load_model(Mzams, Ni, E, R, K, Mix, data_type, extend_tail=False):
    ...
    name = 'M' + str(Mzams) + \
           ...
           '_K' + str(K)
    if data_type == 'lum':
        modelpath = os.path.join('..', 'all_lum_data_no_xg', name, 'lum_observed.dat')
        ...
        modelpath = os.path.join('..', 'all_veloc_data_no_xg', name, 'vel_photo.dat')
        if os.stat(modelpath).st_size < 10 ** 5:
            return 'failed SN'
        else:
            snec_model = pd.read_csv(modelpath,
        ...
        modelpath = os.path.join('..', 'all_temp_rad_data_no_xg', name, 'T_eff.dat')
        if os.stat(modelpath).st_size < 10 ** 5:
            return 'failed SN'
        snec_model = pd.read_csv(modelpath,
...
def load_surrounding_models(requested, ranges_dict, fitting_type, extend_tail=False):
    surrouding_values = mcmc_snec.get_surrouding_values(requested, ranges_dict)
...
def plot_lum_with_fit(data_lum, sampler_df, ranges_dict, output_dir, n_walkers, SN_name, martinez_values, extend_tail=False):
    ...
    for i in range(n_walkers):
        ...
        if mcmc_snec.theta_in_range(requested, ranges_dict) or R == 0:
            ...
            surrounding_values = mcmc_snec.get_surrouding_values(requested[0:6], ranges_dict)
            ...
            if not isinstance(y_fit, str):
                ...
                y_fit = y_fit * S
                if add_label:
                    ax.plot(x_plotting, np.log10(y_fit), alpha=0.2, color='purple', label='SNEC-MCMC fits')
                    add_label = False
                else:
                    ax.plot(x_plotting, np.log10(y_fit), alpha=0.3, color='purple')
    ...
    surrounding_values = mcmc_snec.get_surrouding_values(martinez_values[0:6], ranges_dict)
...
def corner_plot(sampler_chain_flat, ranges_dict, output_dir, martinez_values):
    ...
    # Extract the axes
    axes = np.array(f_corner.axes).reshape((9, 9))
    ...
    for yi in range(8):
        for xi in range(yi):
            ...
            ax.plot(martinez_values[xi], martinez_values[yi], marker='s', color='orange')
...
ranges_dict = {'Mzams' : ranges[0], 'Ni' : ranges[1], 'E' : ranges[2],
               ...
               'S' : ranges[6], 'T' : ranges[7], 'Sv': ranges[8]}
...
flat_sampler_all = pd.read_csv(flat_sampler_path,
                               names=['Mzams', 'Ni', 'E', 'R', 'K', 'Mix', 'S', 'T', 'Sv'],
...
corner_plot(flat_sampler_all, ranges_dict, 'figures', martinez_values)

plot_lum_with_fit(SN_data['lum'], flat_sampler, ranges_dict, 'figures', n_walkers, SN_name, martinez_values)
