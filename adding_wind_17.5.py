...
R_extent = ['1000', '2000', '3000']

for k in range(len(R_extent)):

    mainfolder = os.path.join('profiles_wind_17', 'K{}_R{}'.format(K, R_extent[k]))  # Change
    ...
    for i, imass in enumerate(M_ZAMS):
        ...
        for l in range(10000000):  # (just big number to cover all the profile)
            ...
            # for the wind, and if it is - break
                ...
                mydelta = 4.0 * math.pi / 3.0 * (radius_wind[l] ** 3 -radius_wind[l - 1] ** 3) * rho_wind[l] * rsol ** 3 / msol
        ...
        outfile = open(os.path.join(mainfolder, 's{}'.format(M_ZAMS[i]), 'profiles',
                                    's{}_{}.short'.format(string[i], R_extent[k])), "w")
        ...
        outfile_iso = open(
            mainfolder + '/s' + str(M_ZAMS[i]) + '/profiles' + '/s' + string[i] + '_' + str(R_extent[k]) + '.iso.dat',
            "w")
