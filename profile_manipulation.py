...
def generate_profile_directory_name(imass):
    ...
    if mass_format == 'single':
        profile_basename = os.path.join('sukhbold_profiles', 's{}'.format(str_mass), 'profiles',
                                        's{:2.1f}'.format(imass))
                ...
                for iradius in parameters['wind_extent']:
                    ...
                    for l in range(len(radius_wind)):
                        if l == len(radius_wind) - 1:
                            outfile.write(
                                str(l + 1) + '   ' + str(mass_wind[l] * msol) + '   ' + str(radius_wind[l] * rsol)
                    ...
                    outfile_iso = open(
                        os.path.join(mainfolder, '{}.iso.dat'.format(os.path.basename(base_profile_dir))), "w")
                    ...
                    outfile_iso.write(
                        str(mass_wind[-1] * msol) + '        ' + str(radius_wind[-1] * rsol) + '          ')
