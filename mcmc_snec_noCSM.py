...
def load_model(Mzams, Ni, E, Mix, data_type, extend_tail=False):
    name = 'M' + str(Mzams) + \
           '_Ni' + str(Ni) + \
           '_E' + str(E) + \
           '_Mix' + str(Mix) + \
                ...
                for Mix in surrouding_values['Mix']:
                    if 'lum' in fitting_type:
                        if models['lum'][Mzams][Ni][E][Mix] is None:
                            models['lum'][Mzams][Ni][E][Mix] = load_model(Mzams, Ni, E, Mix, 'lum', extend_tail)
...
def calc_lum_likelihood(theta, data, surrounding_values, extend_tail=False):
    data_x_moved = data['t_from_discovery'] - theta[5]
    data_y = data['Lum']
    data_dy = data['dLum0']
    y_fit = interp.snec_interpolator(theta[0:4], surrounding_values, models['lum'], data_x_moved, extend_tail)
...
def calc_veloc_likelihood(theta, data, surrounding_values, Tthreshold=False):
    ...
    if Tthreshold:
        temp_fit = interp.snec_interpolator(theta[0:4], surrounding_values, models['temp'], data_x_moved)
...
def calc_mag_likelihood(theta, data, surrounding_values):
    log_likeli = 0
    data_x_allfilt = data['t_from_discovery'] - theta[5]
    filters = list(data['filter'].unique())
    y_fit = interp.snec_interpolator(theta[0:4], surrounding_values, models['mag'], data_x_allfilt)
    if not isinstance(y_fit, str):
        # multiply whole graph by scaling factor
        y_fit = y_fit * theta[4]
        ...
        for filt in filters:
            ...
            data_x_filt = data_filt['t_from_discovery'] - theta[5]
...
def log_likelihood(theta, data, ranges_dict, fitting_type, extend_tail=False):
    ...
    if theta_in_range(theta, ranges_dict):
        ...
        load_surrounding_models(theta[0:4], ranges_dict, fitting_type, extend_tail)
        if fitting_type == 'lum':
...
def plot_lum_with_fit(data_lum, sampler_chain, ranges_dict, step, output_dir, n_walkers, SN_name, extend_tail=False):
    ...
    for i in range(n_walkers):
        ...
        if theta_in_range(requested, ranges_dict):
            ...
            y_fit = interp.snec_interpolator(requested[0:4], surrounding_values, models['lum'], data_x_moved, extend_tail)
            if not isinstance(y_fit, str):
                # multiply whole graph by scaling factor
                y_fit = y_fit * S
...
def plot_mag_with_fit(data_mag, sampler_chain, ranges_dict, step, output_dir, n_walkers, SN_name):
    ...
    for i in range(n_walkers):
        ...
        if theta_in_range(requested, ranges_dict):
            ...
            surrounding_values = get_surrouding_values(requested[0:4], ranges_dict)
            y_fit = interp.snec_interpolator(requested[0:4], surrounding_values, models['mag'], data_x_moved_all)
            if not isinstance(y_fit, str):
                # multiply whole graph by scaling factor
                y_fit = y_fit * S
                for filt in filters:
...
def plot_veloc_with_fit(data_veloc, sampler_chain, ranges_dict, step, output_dir, n_walkers, SN_name, Tthreshold=False):
    ...
    for i in range(n_walkers):
        ...
        if theta_in_range(requested, ranges_dict):
            ...
            if Tthreshold:
                temp_fit = interp.snec_interpolator(requested[0:4], surrounding_values, models['temp'], temp_x_moved)
                if not isinstance(temp_fit, str):
                    max_temp_below_Tthresh = np.max(temp_fit[temp_fit <= T_thresh])
            ...
            if len(x_plotting) > 0:
                data_x_moved = x_plotting - T
                y_fit = interp.snec_interpolator(requested[0:4], surrounding_values, models['veloc'], data_x_moved)
                if not isinstance(y_fit, str):
